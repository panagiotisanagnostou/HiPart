...
def split_visualization(hipart_object, color_map="tab20"):
    ...
    additional information related to the algorithm that created it. For the:
        ...
        - dePDDP object, the visualization adds a marginal plot on the X-axis
        that represents the density of the data as extracted from the kernel
        density estimation for the first principal component. That is the
        information the dePDDP algorithm utilizes to split each cluster.

        - kM_PDDP object, the visualization adds a marginal plot on the X-axis
        that represents the data as they are projected on the first principal
        component, with the addition of the centers the k-Means finds within
        its execution on each split. That is the information the kM_PDDP
        algorithm utilizes to split each cluster.

        - iPDDP along with PDDP objects, the visualization adds a marginal plot
        on the X-axis that represents the data as they are projected on the
        first principal component. That is the information the iPDDP algorithm
        utilizes to split each cluster.

        - bicecting_kmeans object does not include additional information in
        the visualization. That is because of the nature of the algorithm it
        implements.
    ...
    dictionary_of_nodes, internal_nodes, _, sample_color = visualization_preparation(
    ...
    gs = gridspec.GridSpec(
        math.ceil(number_of_splits / row_plots) * grid_size, row_plots * grid_size, fig,
    )
    ...
    for i in range(number_of_splits):
        ...
        row_from, row_to, col_from, col_to = grid_position(
            current=i,
            rows=row_plots,
        ...
        if with_hist:
            ...
            if i == 0:
                total_elements = hipart_object.tree.get_node(internal_nodes[i]).data["indices"].shape[0]
            ...
            centers = dictionary_of_nodes[internal_nodes[i]].data["centers"] if isinstance(hipart_object, kM_PDDP) else None
            make_scatter_n_marginal_scatter(
                scatter=scatter,
...
def make_scatter_n_hist(scatter, hist, PP, splitPoint, bandwidth_scale, pr_col, scaler=None):
    """
...
def make_scatter_n_marginal_scatter(scatter, marginal_scatter, PP, splitPoint, pr_col, centers=None):
    """
    Create an Axes plot visualizing the split and data of a cloud of data. With
