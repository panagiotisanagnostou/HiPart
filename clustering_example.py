...
import time
import HIDIV.inteactive_visualization as inteactive_visualization
import HIDIV.visualizations as viz

from sklearn.datasets import make_blobs
from sklearn.metrics import adjusted_rand_score as ari
from sklearn.metrics import normalized_mutual_info_score as nmi
from HIDIV.bisecting_kmeans import bicecting_kmeans
...
depddp = dePDDP(decomposition_method="pca", max_clusters_number=clusters, bandwidth_scale=0.5, percentile=0.1).fit(X)
...
tic = time.perf_counter()
ipddp = iPDDP(decomposition_method="pca", max_clusters_number=clusters, percentile=0.1).fit(X)
toc = time.perf_counter()
ipddp_time = toc - tic
