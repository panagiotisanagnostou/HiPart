# -*- coding: utf-8 -*-
"""
Application of the Principal Direction Divisive Partitioning variation dePDDP algorithm.
...
class dePDDP:
    ...
    decomposition_method : str, optional
        One of the ('pca', 'kpca', 'ica') supported decomposition methods used as kernel for the dePDDP algorithm.
    ...
    **decomposition_args : 
    ...
    fit_predict(X)
        Execute the dePDDP algorithm and return the results of the execution in the form of labels.
    ...
    calculate_node_data(indices, data_matrix, key)
        Calculation of the projections onto the Principal Components with the utilization of the "Principal Components Analysis" or the "Kernel Principal Components Analysis" or the "Indipented Componend Analysis" methods. 
    ...
    """ 
    def __init__(self, decomposition_method='pca', max_clusters_number = 100, bandwidth_scale = 0.5, percentile = 0.1, min_sample_split = 5, **decomposition_args):
    ...
    def select_kid(self, leaves):
        ...
        if len(leaves) > 0:
            for i in sorted(enumerate(leaves), key=lambda x:x[1].data['split_criterion']):
    ...
    def calculate_node_data(self, indices, data_matrix, key):
        ...
        if indices.shape[0] > self.min_sample_split:
            ...
            two_dimensions = util.execute_decomposition_method(data_matrix, self.decomposition_method, self.decomposition_args)
