...
from HIDIV.iPDDP import iPDDP
from HIDIV.kM_PDDP import kM_PDDP
from HIDIV.PDDP import PDDP
...
def main(inputData):
    ...
    obj : dePDDP or iPDDP or kM_PDDP or PDDP object
        The manipulated by the interactive visualization HIDIV object. The object`s type depends on the input object.
    
    """
    
    # Compatibillity check of the input data with the interactive visualization
    if not (isinstance(inputData, dePDDP) or isinstance(inputData, iPDDP) or isinstance(inputData, kM_PDDP) or isinstance(inputData, PDDP)):
        raise TypeError("inputData : can only be instances of classes that belong to PDDP based algoirthm ('dePDDP', 'iPDDP', 'kM-PDDP, PDDP') included in HIDIV package.")
...
def display_menu(pathname, data):
    ...
    -------
    div : dash.html
        A div containing all the visual components needed for each of the page's components.
...
def Splitpoint_Manipulation_Callback(data, curent_figure, split_number, maximum_number_splits, split_marks, splitpoint_position, splitpoint_minimum, splitpoint_max, apply_button):
    ...
    3. the apply button for the change to the new splitpoint (partial algorithm execution from the selected split).
    ...
    data : dict
        The paths of the temporary files created for the execution of the interactive visualization.
    curent_figure : dict
        A dictionary created from the plotly express object plots. It is used for the manipulation of the current figure.
    split_number : int
        The value of the split to project (or is projected, depending on the callback context triggered state).
    maximum_number_splits : int
        The number of splits there are in the model created/manipulated.
    split_marks : dict
        The split number is the key of the dictionary and the assigned value is the value of the dictionary.
    splitpoint_position : float
        The current position of the shape represents the splitpoint of the currently selected split, extracted from the splitpoint slider.
...
def app_layout(app, tmpFileNames): 
    ...
    app.layout = html.Div(children=[
        ...
        html.H1("HIDIV: Interactive Visualisation"),
...
def Cluster_Scatter_Plot(object_path):
    """
    Simple scatter plot creation function. This function is used on the initial visualization of the data and can be accessed throughout the execution of the interactive visualization server.
    
    Parameters
    ----------
    object_path : str
        The location of the temporary file containing the pickel dump of the object we want to visualize.
...
def Splitpoint_Manipulation(object_path):
    """
    Scatter plot with splitpoint visualization creation function. This function is used on the initial visualization of the data and can be accessed throughout the execution of the interactive visualization server.
...
def data_preparation(object_path, splitVal):
    ...
    ----------
    object_path : str
        The location of the temporary file containg the pickel dump of the object we want to visualize.
...
def convert_to_hex(rgba_color):
    """
    Conver the color enconding from RGBa to hexadecimal for integration with the CSS.
...
def recalculate_after_spchange(hidiv_object, split, splitpoint_value):
    """
    Given the serial number of the hidiv algorithm tree`s internal nodes and a new splitpoint value recreate the results of the dePDDP algorithm with the indicated change.
    ...
    for i in dictionary_of_nodes:
        if dictionary_of_nodes[i].is_leaf():
            if dictionary_of_nodes[i].data['split_criterion'] != None:
...
def partial_predict(hidiv_object):
    ...
    while selected_node != None and found_clusters < hidiv_object.max_clusters_number:  # (ST1) or (ST2)
