...
def split_visualization(hidiv_object, color_map = "tab20"):
    """
    Create the visualization of each of the splits generated by one of the divisive hierarchical clustering algorithm members of HIDIV package.
    
    Parameters
    ----------
    hidiv_object : dePDDP or iPDDP or kM_PDDP or PDDP object
        The object member of HIDIV package that we want to manipulate on the premiss of this function.
    color_map : string
        The name of the matplotlib color map to be used for the data visualization.
    
    Raises
    ------
    TypeError
        If the hidiv_object is not a member of the HIDIV package raise an error for the possibility of unexpected errors that will be raised if the elements of the object are not correctly structured.
    ...
    if isinstance(hidiv_object, dePDDP):
        with_hist = True
        grid_size = 4
    elif isinstance(hidiv_object, iPDDP) or isinstance(hidiv_object, kM_PDDP) or isinstance(hidiv_object, PDDP) or isinstance(hidiv_object, bicecting_kmeans):
        with_hist = False
        grid_size = 2
    else:
        raise TypeError("can only process objects of the 'HIDIV' package.")
        
    # prepare the necessary data for this visualization
    dictionary_of_nodes, internal_nodes, _, sample_color = visualization_preparation(hidiv_object, color_map)
    number_of_splits = len(internal_nodes)
    
    # insure that the root node will be always an internal node while is the only node in the tree
    ...
    gs = gridspec.GridSpec(math.ceil(number_of_splits / row_plots) * grid_size, row_plots * grid_size, fig, hspace=.04)
    
    for i in range(number_of_splits):
        # exrtact the subplot position
        row_from, row_to, col_from, col_to = grid_position(current = i, rows = row_plots, splits = number_of_splits, with_hist = with_hist)
        
        # get the color of the sample that belong to the i internal node
        pr_col = sample_color[hidiv_object.tree.get_node(internal_nodes[i]).data['indices']]
        
        # differentiate the visualizations which need explanatory margin plot from simple visualizations
        if with_hist:
            ...
            hist = plt.subplot(gs[row_from:row_to-3, col_from:col_to], sharex=scatter)
            make_sctter_n_hist(scatter=scatter, hist=hist, PP=principal_projections, splitPoint=splitPoint, bandwidth_scale=hidiv_object.split_data_bandwidth_scale, pr_col=pr_col)
            
            hist.title.set_text("Original data with 1st split") if i == 0 else hist.title.set_text("Split no. " + str(i+1))
            
        else:
            # bicecting k-Means doesn't execute PCA so we execute it here for the visualization
            if isinstance(hidiv_object, bicecting_kmeans):
                principal_projections = util.execute_decomposition_method(hidiv_object.X[dictionary_of_nodes[internal_nodes[i]].data['indices']], 'pca', {})
                show_split = False
                splitPoint = None
            else:
                principal_projections = dictionary_of_nodes[internal_nodes[i]].data['projection']
            ...
            ax = make_simple_scatter(sp=ax, splitPoint=splitPoint, PP=principal_projections, pr_col=pr_col, show_split = show_split)
...
def make_sctter_n_hist(scatter, hist, PP, splitPoint, bandwidth_scale, pr_col):
    """
    Create an Axes plot visualizing the split and data of a cloud of data.
    ...
    """
    bandwidth = sm.nonparametric.bandwidths.select_bandwidth(PP[:,0], "silverman", kernel=None)
    s, e = FFTKDE(kernel='gaussian', bw = (bandwidth_scale * bandwidth)).fit(PP[:,0]).evaluate()
...
def visualization_preparation(hidiv_object, color_map):
    """
    ...
    dictionary_of_nodes : list
        The list of all the nodes created by the splitting of the data that were performed by the HIDIV member algorithms.
    internal_nodes : list
        The list of internal nodes of the binary tree that were created by the splitting of the data performed by the HIDIV algorithms.
    color_list : list
        An array containing the colors to be used by the visualization as RGBa tuple.
    sample_color : numpy.ndarray
        An array containing the color of each sample as RGBa tuple.

    """
    
    dictionary_of_nodes = hidiv_object.tree.nodes
    ...
    # find the clusters from the tree generated by the divisive clustering algorithms
    clusters = hidiv_object.tree.leaves()
    clusters = sorted(clusters, key = lambda x:x.identifier)
    
    # create colormap for the generated clusters
    cluster_map = np.zeros(hidiv_object.samples_number)
    for i in clusters:
...
def grid_position(current, rows, splits, with_hist=True):
    ...
    # Change the last row of the visualization to have the subplot always in the middle of the plot
...
def dendrogram_visualization(hidiv_object, **dendrogram_parameters):
    """
    Create a dendrogram visualization of the divive clustering based on the HIDIV`s alogriths execution.

    Parameters
    ----------
    hidiv_object : dePDDP or iPDDP or kM_PDDP or PDDP object
        The object member of HIDIV package that we want to manipulate on the premiss of this function.
    **dendrogram_parameters : TYPE
        All the parameters the scipy.cluster.hierarchy.dendrogram function can take except the color_treshold parameter.
    ...
    if not (isinstance(hidiv_object, dePDDP) or isinstance(hidiv_object, iPDDP) or isinstance(hidiv_object, kM_PDDP) or isinstance(hidiv_object, PDDP) or isinstance(hidiv_object, bicecting_kmeans)):
        raise TypeError("can only process objects of the 'HIDIV' package.")
    
    Z = create_linkage(hidiv_object.tree)
    
    return dendrogram(Z, color_threshold=0.3, **dendrogram_parameters)


def create_linkage(tree_in):
    """
    Create the linkage matrix for the encoding of the divisive clustring tree creatred by the member algorithms of the HIDIV package.

    Parameters
    ----------
    tree_in : treelib.tree.Tree
        A divisive tree from on of the HIDIV algorithm package.
    ...
    Z = np.array([[0,0,0,0]]) 
    # Loop through the nodes of the algorithm`s execution tree and do the necessary connections
    # The loop finishes the execution on the node with ID 0 which is always the root of the algorithm execution tree
    for i in range(len(tree.nodes)-1,-1,-1):
        # If the node is a leaf of the algorithm`s execution tree connect all the samples of the node on same level untile only one node remains
        if tree.get_node(i).is_leaf():
            if not tree.get_node(i).data["dendrogram_check"]:
                # Set all the samples of the included in the node/cluster as unlinked nodes on the dendrogram tree
                tree.get_node(i).data["unlinked_nodes"] = tree.get_node(i).data["indices"]
                # Create the dendrograms subtree and update the algorithm tree node`s data and the index for the next free node
                cluster_linkage, tree.get_node(i).data, dendrogram_counts = linkage_data_maestro(tree.get_node(i), dendrogram_counts, 0.2)
                dendrogram_counts += 1
                Z = np.vstack((Z, cluster_linkage))
        else:
            if not tree.get_node(i).data["dendrogram_check"]:
                # Connect the children of the algorithm tree internal node to a new node on the dendrogram tree
                children = tree.children(i)
                Z = np.vstack([Z, [children[-1].data["unlinked_nodes"][0], 
                ...
                tree.get_node(i).data["counts"] = children[-1].data["counts"] + children[-2].data["counts"]
                tree.get_node(i).data["unlinked_nodes"] = [dendrogram_counts]
                dendrogram_counts += 1
    
    # Remove the first row of the linkage matrix bacause it is the initalixation`s row of zeros
    Z = Z[1:,:]
...
def _get_node_depth(path_to_leaves, i):
    """
    This function calculates the depth of the node i inside the algorithm`s execution tree.
...
def linkage_data_maestro(node, dendrogram_counts, distance):
    """
    Manages the process of the dendrogram`s subtree creation for a cluster extracted from a divisive algorithm of the HIDIV package. This process includes the indication of the unlinked nodes included in the cluster (the leaf node of the divisive algorithm`s tree) for the dendrogram tree.

    Parameters
    ----------
    node : treelib.node.Node
        The algorim tree node we want to create a subtree for the dendrogram tree from.
    ...
    # Update the indicator of the node of the algorithm`s tree which indicates that the node has been added to the dendrogram`s tree
    node.data["dendrogram_check"] = True
    
    leave = node.data["unlinked_nodes"]
    cluster_linkage, dendrogram_counts = create_cluster_linkage(leave, np.ones(len(leave)), dendrogram_counts, distance)
...
def create_cluster_linkage(points, samples_count, node_indicator, distance):
    """
    A recursive function that creates the linkage of the subtree dendrogram tree. The execution of the function ends when only one unliked node exists in the subtree.
    ...
    # If the number of the unlinked nodes are odd seperate the last unlinked node and add it afterword as a new subtree
    if (elements % 2) == 0:
        # Create the linkage of the points
        Z, connections = create_the_connections(points, samples_count, node_indicator, distance)
        counts = Z[:,3]
    else:
        ...
        Z = np.vstack([Z, [forever_alone, alone_conection, distance, Z[0,3] + alone_count]])
        node_indicator += 1
        counts[0] += alone_count
    
    # Recursive call for the next level unliked created
    new_nodes = np.array([ i for i in range(node_indicator, node_indicator + connections) ])
    node_indicator += connections
    next_Z, dendrogram_counts = create_cluster_linkage(new_nodes, counts, node_indicator, distance)
    
    # Either return the linkage create with the node indicator for the next free node (end of the recursive call) or merge the previous linkage lower level linkage with the new higher one
    if next_Z is None:
...
def create_the_connections(points, samples_count, node_indicator, distance):
    ...
    return np.column_stack((left, right, distance, samples_count)), elements//2
